...
void weapon_chainfist_fire(edict_t *ent)
{
	...
	//	damage = 15;
	int damage = chainfist_damage_sp->integer;
	if (deathmatch->integer)
		damage = chainfist_damage->integer;
	// Kyper
	...
	if (fire_player_melee(ent, start, dir, chainfist_range->integer, damage, 100, MOD_CHAINFIST))
	// Kyper
	{
...
void weapon_tracker_fire(edict_t *self)
{
	...
	//	damage = 135;
	if (deathmatch->integer)
		damage = disruptor_damage->integer;
	else
		damage = disruptor_damage_sp->integer;
	// Kyper
	...
	fire_tracker(self, start, dir, damage, disruptor_speed->integer, enemy);
	// Kyper
...
void weapon_etf_rifle_fire(edict_t *ent)
{
	...
	//else
	//	damage = 10;
	damage = etfrifle_damage->integer;
	// Kyper
	...
	fire_flechette(ent, start, dir, damage, etfrifle_speed->integer, kick);
	// Kyper
...
void Heatbeam_Fire(edict_t *ent)
{
	...
	// Kyper - Lithium port - DM and SP damage are both 15
	//if (deathmatch->integer)
	//	damage = HEATBEAM_DM_DMG;
	//else
	//	damage = HEATBEAM_SP_DMG;
	damage = plasmabeam_damage->integer;
	// Kyper
	...
	//	kick = 30;
	if (deathmatch->integer)
		kick = plasmabeam_kick->integer;
	else
		kick = plasmabeam_kick_sp->integer;
	// Kyper
