...
TOUCH(rocket_touch) (edict_t *ent, edict_t *other, const trace_t &tr, bool other_touching_self) -> void
{
	...
	if (other->takedamage)
	{
		...
		T_Damage(other, ent, ent->owner, ent->velocity, ent->s.origin, tr.plane.normal, ent->dmg, rocket_knockback->integer, DAMAGE_NONE, MOD_ROCKET);
		// Kyper
	}
	else
	{
...
TOUCH(bfg_touch) (edict_t *self, edict_t *other, const trace_t &tr, bool other_touching_self) -> void
{
	...
	// Kyper - Lithium port
		...
		T_Damage(other, self, self->owner, self->velocity, self->s.origin, tr.plane.normal, bfg_balldamage->integer, 0, DAMAGE_ENERGY, MOD_BFG_BLAST);
	//T_RadiusDamage(self, self->owner, 200, other, 100, DAMAGE_ENERGY, MOD_BFG_BLAST);
	T_RadiusDamage(self, self->owner, bfg_balldamage->integer, other, bfg_ballradius->integer, DAMAGE_ENERGY, MOD_BFG_BLAST);
	// Kyper
...
THINK(bfg_think) (edict_t *self) -> void
{
	...
	//	dmg = 10;
	if (deathmatch->integer)
		dmg = bfg_raydamage->integer;
	else
		dmg = bfg_raydamage_sp->integer;
	// Kyper
