			  ...
			  const vec3_t &normal, int damage, int knockback, damageflags_t dflags, mod_t mod)
{
	...
	if (g_instagib->integer && attacker->client && targ->client && mod.id != MOD_HOOK)   // Kyper - Lithium port - don't instakill with hook
	{
		// [Kex] always kill no matter what on instagib
		damage = 9999;
	}
	// Kyper - Lithium port - only bother with damage calculations outside of instagib
	else
	{
