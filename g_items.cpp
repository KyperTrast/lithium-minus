...
THINK(MegaHealth_think) (edict_t *self) -> void
{
	...
	if (self->owner->health > self->owner->max_health)
	{
		...
		if (!(Rune_HasRegen(self->owner) && self->owner->health < rune_regenmax->value + 1))
			self->owner->health -= 1;
		return;
	}
	// Kyper
...
void InitItems()
{
	...
	for (auto &it : itemlist)
	{
		...
		else if (deathmatch->integer && !g_dm_weapons_stay_fixdrop->integer) // Kyper - Lithium port - fix weapons not dropping on death
		{
