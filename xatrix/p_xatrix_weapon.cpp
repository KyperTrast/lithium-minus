...
void weapon_ionripper_fire(edict_t *ent)
{
	...
	//	damage = 50;
	if (deathmatch->integer)
		damage = ionripper_damage->integer;
	else
		damage = ionripper_damage_sp->integer;
	// Kyper
	...
	fire_ionripper(ent, start, dir, damage, ionripper_speed->integer, EF_IONRIPPER);
	// Kyper
...
void weapon_phalanx_fire(edict_t *ent)
{
	...
	//radius_damage = 120;
	//damage_radius = 120;
	damage = irandom(phalanx_damage->integer, phalanx_damage->integer + phalanx_damage2->integer);
	radius_damage = phalanx_radius->integer;
	damage_radius = phalanx_rdamage->integer;
	// Kyper
	...
	if (ent->client->ps.gunframe == 8)
	{
		...
		radius_damage = phalanx_radius2->integer;
		damage_radius = phalanx_rdamage2->integer;
		fire_plasma(ent, start, dir, damage, phalanx_speed->integer, damage_radius, radius_damage);
		// Kyper
