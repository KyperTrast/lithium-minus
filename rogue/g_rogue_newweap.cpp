...
THINK(Prox_Explode) (edict_t *ent) -> void
{
	...
	if (ent->dmg > proxlauncher_damage->integer)     // Kyper - Lithium port
		gi.sound(ent, CHAN_ITEM, gi.soundindex("items/damage3.wav"), 1, ATTN_NORM, 0);

	ent->takedamage = false;
	// Kyper - Lithium port
	//T_RadiusDamage(ent, owner, (float) ent->dmg, ent, PROX_DAMAGE_RADIUS, DAMAGE_NONE, MOD_PROX);
	T_RadiusDamage(ent, owner, (float)ent->dmg, ent, proxlauncher_radius->value, DAMAGE_NONE, MOD_PROX);
	// Kyper
...
THINK(prox_open) (edict_t *ent) -> void
{
	...
	if (ent->s.frame == 9) // end of opening animation
	{
		...
		while ((search = findradius(search, ent->s.origin, proxlauncher_radius->value + 10)) != nullptr)     // Kyper - Lithium port
		{
		...
		else
		{
			...
			switch (ent->dmg / proxlauncher_damage->integer)   // Kyper - Lithium port
			{
			case 1:
				ent->wait = (level.time + PROX_TIME_TO_LIVE).seconds();
				break;
...
void fire_prox(edict_t *self, const vec3_t &start, const vec3_t &aimdir, int prox_damage_multiplier, int speed)
{
	...
	prox->dmg = proxlauncher_damage->integer * prox_damage_multiplier;
	// Kyper
...
void tesla_remove(edict_t *self)
{
	...
	if ((self->dmg_radius) && (self->dmg > (tesla_damagerate->integer * TESLA_EXPLOSION_DAMAGE_MULT)))         // Kyper - Lithium port
...
THINK(tesla_think_active) (edict_t *self) -> void
{
	...
	for (i = 0; i < num; i++)
	{
		...
		if (tr.fraction == 1 || tr.ent == hit)
		{
			...
			if (self->dmg > tesla_damagerate->integer)          // Kyper - Lithium port
				gi.sound(self, CHAN_ITEM, gi.soundindex("items/damage3.wav"), 1, ATTN_NORM, 0);
...
void fire_tesla(edict_t *self, const vec3_t &start, const vec3_t &aimdir, int tesla_damage_multiplier, int speed)
{
	...
	tesla->dmg = tesla_damagerate->integer * tesla_damage_multiplier;            // Kyper - Lithium port
...
THINK(tracker_pain_daemon_think) (edict_t *self) -> void
{
	...
	if ((level.time - self->timestamp) > (TRACKER_DAMAGE_TIME - gtime_t::from_ms(disruptor_fixticks->integer ? 1 : 0)))
	// Kyper
	{
